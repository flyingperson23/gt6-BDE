		...
		if (rRecipe == null) {
			...
			for (ItemStack aInput : aInputs) if (ST.valid(aInput)) {
				...
				if (ST.valid(tUSB) && ST.valid(tScanned)) {
					...
					if (((!DISABLE_MISC && tScanned.getItem() == ItemsGT.BOOKS) || OD.bookWritten.is_(tScanned) || IL.Paper_Printed_Pages.equal(tScanned, F, T) || IL.Paper_Printed_Pages_Many.equal(tScanned, F, T)) && UT.Code.stringValid(UT.NBT.getBookTitle(tScanned))) {
