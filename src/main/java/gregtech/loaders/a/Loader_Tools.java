...
public class Loader_Tools implements Runnable {
	...
	public void run() {
		...
		if (!DISABLE_MISC) ToolsGT.add(ToolsGT.sMetaTool.addTool(ToolsGT.CLUB                      , "Club"                            , "A blunt primitive weapon with a lot of Power"        , new GT_Tool_Club()                                             .setMaterialAmount(6*U                                       )                                                                                                                                           , TC.stack(TC.INSTRUMENTUM  , 2), TC.stack(TC.HUMANUS       , 2), TC.stack(TC.STRONTIO      , 2)                                ), TOOL_hammer);
...
//      ToolsGT.add(ToolIDs.sMetaTool.addTool(ToolIDs.SICKLE                    , "Sickle"                          , "Harvests Crops more efficiently"                     , new GT_Tool_Sickle()                                           .setMaterialAmount(3*U                                       ), OreDictToolNames.blade                                                                                                                   , TC.stack(TC.INSTRUMENTUM  , 2), TC.stack(TC.METO          , 2), TC.stack(TC.GRANUM        , 2)                                ), TOOL_sickle);
		...
		if (!DISABLE_MISC) ToolsGT.add(ToolsGT.sMetaTool.addTool(ToolsGT.FLINT_AND_TINDER          , "Flint and Tinder"                , ""                                                    , new GT_Tool_FlintAndTinder()                                   .setMaterialAmount(0                                         ), OreDictToolNames.flintandtinder, OD.craftingFirestarter                                                                                  , TC.stack(TC.INSTRUMENTUM  , 2), TC.stack(TC.IGNIS         , 2)                                                                ), TOOL_igniter);
		if (!DISABLE_MISC) ToolsGT.add(ToolsGT.sMetaTool.addTool(ToolsGT.MONKEY_WRENCH             , "Monkey Wrench"                   , "Hold Leftclick to dismantle Machines"                , new GT_Tool_MonkeyWrench()                                     .setMaterialAmount(6*U                                       ), OreDictToolNames.monkeywrench                                                                                                            , TC.stack(TC.INSTRUMENTUM  , 2), TC.stack(TC.MACHINA       , 2), TC.stack(TC.ORDO          , 2)                                ), TOOL_monkeywrench);
		...
		if (!DISABLE_MISC) 			ToolsGT.sMetaTool.addTool(ToolsGT.BENDING_CYLINDER_SMALL    , "Small Bending Cylinder"          , ""                                                    , new GT_Tool_BendingCylinderSmall()                             .setMaterialAmount(3*U                                       ), OreDictToolNames.bendingcylindersmall                                                                                                    , TC.stack(TC.INSTRUMENTUM  , 1), TC.stack(TC.METALLUM      , 2)                                                                );
		...
		if (!DISABLE_MISC) ToolsGT.add(ToolsGT.sMetaTool.addTool(ToolsGT.MAGNIFYING_GLASS          , "Magnifying Glass"                , "Crafted with a Stick and a Lens"                     , new GT_Tool_MagnifyingGlass()                                  .setMaterialAmount(lens                              .mAmount), OreDictToolNames.magnifyingglass                                                                                                         , TC.stack(TC.INSTRUMENTUM  , 1), TC.stack(TC.SENSUS        , 2)                                                                ), TOOL_magnifyingglass);
		...
		if (!DISABLE_MISC) ToolsGT.add(ToolsGT.sMetaTool.addTool(ToolsGT.GEM_PICK                  , "Gem tipped Pickaxe"              , "Quarter of the Durability of a full Gem Pick"        , new GT_Tool_PickaxeGem()                                       .setMaterialAmount(toolHeadPickaxeGem                .mAmount), OreDictToolNames.pickaxe                                                                                                                 , TC.stack(TC.INSTRUMENTUM  , 3), TC.stack(TC.PERFODIO      , 3)                                                                ), TOOL_pickaxe);
		...
		for (int i = 0; i < 8 && !DISABLE_MISC; i++) ItemsGT.addNEIRedirect(
		...
		toolHeadBuilderwand        .addListener(new OreProcessing_Tool(BUILDERWAND           , tCategory + "Builderwand"         ,T,F,0,0, null, null                                                                                , new String[][] {{" P ", "f h", " s "}, {" C ", "f h", " s "}, {" G ", "f h", " s "}}, null         , null, null, null                              , null, new And(ANTIMATTER.NOT)));
			if (!DISABLE_MISC) toolHeadConstructionPickaxe.addListener(new OreProcessing_Tool(CONSTRUCTION_PICK     , tCategory + "ConstructionPickaxe" ,T,F,0,0, null, null                                                                                , new String[][] {{"PIP", "f h"       }, {"CGC", "f  "       }                       }, null         , null, null, null                              , null, new And(ANTIMATTER.NOT, MT.Wood.NOT, COATED.NOT)));
		...
		toolHeadHammer             .addListener(new OreProcessing_Tool(CLUB                  , tCategory + "Club"                ,T,F,0,0, null, new String[][] {{" II", "III", "HI "}, {" GG", "GGG", "HG "}, {" RR", "RRR", "HR "}}, null                                                                                , null         , null, null, null                              , null, new And(ANTIMATTER.NOT, MT.Wood.NOT)));
			if (!DISABLE_MISC) toolHeadArrow              .addListener(new OreProcessing_Tool(HAND_DRILL            , tCategory + "HandDrill"           ,T,F,0,0, null, new String[][] {{"  X", "HYH", "YH "}}                                              , null                                                                                , toolHeadArrow, bolt, null, null                              , null, new And(ANTIMATTER.NOT, MT.Wood.NOT, WOOD.NOT, BOUNCY.NOT, STRETCHY.NOT, typemin(2), qualmin(2))));

		if (!DISABLE_MISC) GameRegistry.addRecipe(new AdvancedCraftingTool(MAGNIFYING_GLASS , lens                       , typemin(1), MT.Glass));
		...
		GameRegistry.addRecipe(new AdvancedCraftingTool(BUILDERWAND      , toolHeadBuilderwand        , MT.YellowSapphire)); // Suggests using Gems for those Wands since you usually only have one workable Gem per Type.
			if (!DISABLE_MISC) GameRegistry.addRecipe(new AdvancedCraftingTool(CONSTRUCTION_PICK, toolHeadConstructionPickaxe));
			if (!DISABLE_MISC) GameRegistry.addRecipe(new AdvancedCraftingTool(GEM_PICK         , toolHeadPickaxeGem         , MT.Amber)); // Amber, to show the Silk Touch usage first.
		...
		if (!DISABLE_MISC) toolHeadScrewdriver.addListener(new OreProcessing_Tool(POCKET_MULTITOOL    , tCategory + "PocketMultitool",F,F, 0,   0, MT.Blue  , new String[][] {{"AXO", "ZPV", "OWY"}}, null, toolHeadSaw, toolHeadChisel, toolHeadFile, toolHeadSword, toolHeadSword, tCondition));
